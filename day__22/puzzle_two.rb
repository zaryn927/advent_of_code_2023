...
def main
  ...
  supported_map = Hash.new { |hash, key| hash[key] = Set[] }
  supporters_map = Hash.new { |hash, key| hash[key] = Set[] }
  bricks.each do |brick|
    supports(supporters_map, supported_map, bricks, brick)
  end
  total = 0
  bricks.each do |brick|
    removed = Set[]
    remove_brick(removed, supporters_map, supported_map, brick)
...
def remove_brick(removed, supporters_map, supported_map, brick)
  removed << brick
  supported = supported_map[brick]
  supported.each do |sup_brick|
    supporters = supporters_map[sup_brick]
    remove_brick(removed, supporters_map, supported_map, sup_brick) if supporters.all? { |s| removed.include?(s) }
  end
end

# determine which bricks depend on and are depended on by others
def supports(supporters_map, supported_map, bricks, brick)
  supported = bricks.filter { |b| b.lz == (brick.rz + 1) && overlap?(b, brick) }
  supported.each do |sup_brick|
    supported_map[brick] << sup_brick
    supporters = bricks.filter { |b| b.rz == (sup_brick.lz - 1) && overlap?(b, sup_brick) }
    supporters.each { |s| supporters_map[sup_brick] << s }
    supporters_map[sup_brick].flatten!
  end
end
